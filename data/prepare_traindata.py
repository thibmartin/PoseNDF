...
def faiss_idx_np(root_dir, bm, device='cpu', batch_size=128):#NO GPU on my laptop
    all_joints = []
    all_pose = []
    ...
    ds_dir= os.path.join(root_dir, "OH0037_body")
...
def faiss_idx_torch(root_dir):
    ...
    for seq in seqs:
        ...
        data_val = np.load(os.path.join(ds_dir, seq))['pose_body'].astype(np.float32)[:, :63]
    ...
    data_all = torch.cat(all_pose).to(device='cpu')
...
def main(args):
    device ='cpu'# NO GPU available on my laptop
    #read the pose data:
    ...
    bm_path='C:/Users/thiba/Desktop/Semester_project/PoseNDF/SMPL_python_v.1.1.0' #path to the SMPL models folder
    ...
    #amass_datas = sorted(amass_splits['train']) # useless now

    if args.data_type == 'np':
        faiss_model, _, all_pose = faiss_idx_np(args.raw_data, bm, batch_size=500)
    else:
        faiss_model, _, all_pose = faiss_idx_torch(args.raw_data)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-rd", "--raw_data", type=str, default="C:/Users/thiba/Desktop/Semester_project/PoseNDF/data",
                        help="Path to the resulting image")
    parser.add_argument("-od", "--out_dir", type=str,
                        default="C:/Users/thiba/Desktop/Semester_project/PoseNDF/data/training_data",
                        help="Path to the resulting image")
    ...
    parser.add_argument("-sf", "--seq_file", type=str,
                        default="OH0037_body/pose3d_np_body.npz",
                        help="sequence file")
